export async function POST(req) {
  ...
  const cluster_number = searchParams.get('cluster_number');
  ...
  try {
    ...
    console.log("Cluster number for reset:", cluster_number);
    const response = await fetch(
      `${serverBaseUrl}/api/clusters/${projectId}/${cluster_number}/reset`,
      {
        method: "POST",
        headers: {
    ...
    return new Response(
      JSON.stringify({ success: false, error: "Failed to reset cluster" }),
      { status: 500 }
    );
  }
}
